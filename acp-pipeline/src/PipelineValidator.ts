...
const BRANCH_OUTPUT_RE =
	/^steps\.([A-Za-z0-9_-]+)\.branches\.([A-Za-z0-9_-]+)\.output$/;
...
export function validatePipelineDefinition(
	...
	if (!isRecord(primitivesValue)) {
		...
		for (const [primitiveId, primitiveValue] of Object.entries(
			primitivesValue,
		)) {
			...
			const agent = readRequiredString(
				primitiveValue,
				"agent",
				errors,
			...
			const promptFile = readOptionalString(
				primitiveValue,
				"promptFile",
				errors,
				`primitive "${primitiveId}"`,
			...
			if (output !== "markdown" && output !== "proposed_plan") {
				errors.push(
					`primitive "${primitiveId}" output must be "markdown" or "proposed_plan".`,
				);
			}
			...
			if (agent && !agentConfigs[agent]) {
				errors.push(
					`primitive "${primitiveId}" references missing ACP agent "${agent}".`,
				);
			}
			if (
				agent &&
...
function readSteps(
	...
	for (const [index, stepValue] of stepsValue.entries()) {
		...
		if (typeof use === "string") {
			if (type !== undefined) {
				errors.push(
					`step "${stepId || index + 1}" cannot have both use and type.`,
				);
			}
			if (!primitives[use]) {
				errors.push(
					`step "${stepId || index + 1}" references unknown primitive "${use}".`,
				);
			}
			if (stepId) {
		...
		if (type === "parallel") {
			const branches = readParallelBranches(
				stepValue.branches,
		...
		errors.push(
			`step "${stepId || index + 1}" must define use, type: approval, or type: parallel.`,
		);
	}
...
function readParallelBranches(
	...
	for (const [index, branchValue] of branchesValue.entries()) {
		if (!isRecord(branchValue)) {
			errors.push(
				`parallel step "${stepId}" branch ${index + 1} must be an object.`,
			);
			continue;
		}
		const branchId = readRequiredString(
			branchValue,
			"id",
			errors,
			`parallel step "${stepId}" branch ${index + 1}`,
		);
		const use = readRequiredString(
			branchValue,
			"use",
			errors,
			`parallel step "${stepId}" branch "${branchId || index + 1}"`,
		);
		if (branchId && seenBranchIds.has(branchId)) {
			errors.push(
				`parallel step "${stepId}" branch id "${branchId}" is duplicated.`,
			);
		}
		if (branchId) {
		...
		if (use && !primitives[use]) {
			errors.push(
				`parallel step "${stepId}" branch "${branchId || index + 1}" references unknown primitive "${use}".`,
			);
		}
		if (use && primitives[use]?.sideEffects === "workspace") {
			errors.push(
				`parallel step "${stepId}" branch "${branchId || index + 1}" cannot use workspace side effects.`,
			);
		}
		if (branchId && use) {
	...
	for (const variable of extractTemplateVariables(template)) {
		...
		if (stepMatch) {
			if (!previousStepOutputs.has(stepMatch[1])) {
				errors.push(
					`${scope} template variable "{{${variable}}}" must reference a previous step.`,
				);
			}
			continue;
		}
		...
		if (branchMatch) {
			if (!previousBranchOutputs.has(`${branchMatch[1]}.${branchMatch[2]}`)) {
				errors.push(
					`${scope} template variable "{{${variable}}}" must reference a previous parallel branch.`,
				);
			}
			continue;
		}
		errors.push(
			`${scope} template variable "{{${variable}}}" is not supported.`,
		);
	}
}
...
function validatePrimitiveTemplateSyntax(
	...
	for (const [primitiveId, primitive] of Object.entries(primitives)) {
		...
		for (const variable of extractTemplateVariables(primitive.prompt)) {
			if (
				variable === "userPrompt" ||
				STEP_OUTPUT_RE.test(variable) ||
				BRANCH_OUTPUT_RE.test(variable)
			) {
				continue;
			}
			errors.push(
				`primitive "${primitiveId}" uses unsupported template variable "{{${variable}}}".`,
			);
		}
	}
}
...
function readOptionalStringArray(
	...
	if (
		...
		errors.push(
			`${scope} ${key} must be an array of non-empty strings when provided.`,
		);
		return null;
	}
