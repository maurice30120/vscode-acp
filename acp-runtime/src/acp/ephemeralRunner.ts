...
import { loadAgentCatalog } from "../catalog/config.js";
import {
	loadSkillCatalog,
	renderSkillsCatalog,
...
export interface EphemeralAcpRunnerOptions {
	getAgentConfigs?: () => Record<string, AgentConfigEntry>;
...
export class EphemeralAcpRunner {
	...
	private readAgentConfig(agentName: string): AgentConfigEntry {
		const configs =
			this.options.getAgentConfigs?.() ??
			loadAgentCatalog(this.workspaceCwd).agents;
...
function isSandcastleConfig(config: AgentConfigEntry): config is SandcastleAgentConfig {
