...
export default function acpPipelinePiExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      const result = await getController(ctx.cwd).runPipeline(params.pipelineName ?? '', params.prompt, ctx);
...
export { handlePipelineCommand, parseRunArgs, registerPipelineCommand } from './runtime/commands.js';
...
export { getPipelineDefinitions, loadWorkspacePipelineDefinitions, parsePipelineYaml } from './catalog/pipelineCatalog.js';
export { loadWorkspaceTeamEntries } from './catalog/teamCatalog.js';
export { InstructionResolver } from './catalog/instructionResolver.js';
