...
import { createTempWorkspace, writeDefaultConfig, writeDemoPipeline } from './helpers.js';
  ...
  const runner = new EphemeralAcpRunner(workspace, {
    ...
    connector: async () => ({
      ...
      connInfo: {
        ...
        connection: {
          newSession: async () => ({ sessionId: 's1' }),
          prompt: async () => new Promise<{ stopReason: 'cancelled' }>(resolve => {
...
test('/pipeline list reports configured pipelines', async () => {
  ...
  const controller = new PipelineController(workspace, { sendMessage: () => {} } as any, {
    runner: { run: async () => '<proposed_plan>unused</proposed_plan>' },
  });

  await handlePipelineCommand('list', commandContext(workspace, notifications), controller);

  assert.match(notifications[0], /Demo Pipeline/);
});
...
test('/pipeline run then approve executes planner and implementer', async () => {
  ...
  const controller = new PipelineController(workspace, {
  ...
  await handlePipelineCommand('run Demo Pipeline add tests', ctx, controller);
