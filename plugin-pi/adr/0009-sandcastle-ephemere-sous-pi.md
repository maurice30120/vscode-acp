...
2. **Config séparée du workspace** sous `.acp/.sandcastle/config.json` : top-level `promotion` (`ask` | `autoApply` | `autoReject`) + `agents` (Sandcastle-only, chaque entrée portant `transport: 'sandcastle'` requis). Providers supportés : `codex`, `cursor`, `pi`, `vibe`. Le plugin n’embarque aucune copie de cette configuration. `acp-agents.json` reste natif-only ; si `transport: 'sandcastle'` y apparaît → rejet pointant vers le fichier dédié. Un doublon de nom d'agent entre les deux fichiers est une erreur.
