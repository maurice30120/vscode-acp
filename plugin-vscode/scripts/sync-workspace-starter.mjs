...
const destRoot = path.join(extensionRoot, 'resources', 'workspace-starter');
...
function copyAcpPipelines() {
  const pipelinesDir = path.join(workspaceRoot, '.acp', 'pipelines');
...
function writeManifest() {
  let sourceCommit;
  try {
    sourceCommit = execSync('git rev-parse HEAD', { cwd: workspaceRoot, encoding: 'utf8' }).trim();
  } catch {
...
copyTree(path.join(workspaceRoot, '.agents'), '.agents');
