...
export function normalizeCodexModelsCacheForLegacyCli(): void {
  ...
  try {
    ...
    if (changed) {
      ...
      log('Normalized Codex models cache for legacy codex-acp compatibility.');
    }
  } catch (error) {
    logError('Failed to normalize Codex models cache for legacy codex-acp compatibility', error);
  }
}
