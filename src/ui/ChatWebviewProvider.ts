...
/**
 * WebviewViewProvider pour le panneau lateral de chat ACP.
 * Rend les messages, appels d'outils et plans, puis gere les interactions utilisateur.
 */
export class ChatWebviewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(
    ...
    // Configure marked pour un rendu Markdown robuste
    marked.setOptions({
      breaks: true,
      gfm: true,
    });

    // S'enregistre comme listener des mises a jour de session
  ...
  /**
   * Convertit du Markdown en HTML avec marked.
   */
  ...
  resolveWebviewView(
    ...
    // Gere les messages entrants depuis le webview
    webviewView.webview.onDidReceiveMessage(async (message) => {
      switch (message.type) {
        ...
        case 'executeCommand':
          ...
          break;
        case 'ready':
          // Le webview est charge : envoyer l'etat courant de session
          this.sendCurrentState();
          break;
        case 'renderMarkdown': {
          // Le webview demande le rendu Markdown pour les elements d'historique
  ...
  /**
   * Relaye une mise a jour de session vers le webview.
   */
  private handleSessionUpdate(update: SessionNotification): void {
    // Ne relaye que les mises a jour de la session active
    const activeId = this.sessionManager.getActiveSessionId();
    if (update.sessionId !== activeId) { return; }

    // Persiste les commandes disponibles dans l'etat de session local
  ...
  /**
   * Traite un prompt envoye depuis le webview.
   */
  private async handleSendPrompt(text: string): Promise<void> {
    ...
    // Informe le webview qu'un traitement est en cours
    this.postMessage({ type: 'promptStart' });

    try {
      const response = await this.sessionManager.sendPrompt(activeId, text);
      // Rend le texte assistant accumule en Markdown
      // Le webview a deja envoye le texte brut via la sequence promptEnd
      this.postMessage({
        type: 'promptEnd',
  ...
  /**
   * Traite une demande d'annulation issue du webview.
   */
  ...
  /**
   * Traite un changement de mode depuis le selecteur du webview.
   */
  ...
  /**
   * Traite un changement de modele depuis le selecteur du webview.
   */
  ...
  /**
   * Envoie l'etat courant de session au chargement du webview.
   */
  ...
  /**
   * Poste un message au webview s'il est instancie.
   */
  ...
  /**
   * Notifie le webview qu'une nouvelle session est active.
   */
  ...
  /**
   * Notifie le webview d'un changement d'etat de mode.
   */
  ...
  /**
   * Notifie le webview d'un changement d'etat de modele.
   */
  ...
  /**
   * Efface l'historique du chat et revient a l'etat d'accueil.
   * Appele lorsqu'une nouvelle conversation est lancee.
   */
  clearChat(): void {
  ...
  /**
   * Indique si le chat contient deja des messages.
   */
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Corps Markdown dans les messages assistant */
    ...
    /* Bloc de reflexion : element <details> repliable */
    .thought-block {
      width: 100%;
      margin-bottom: 4px;
    }
    ...
    /* Conteneur de tour : groupe texte assistant + appels d'outils */
    .turn {
      display: flex;
      flex-direction: column;
      gap: 4px;
    ...
    /* Groupe d'appels d'outils a l'interieur d'un tour */
    .turn-tools {
      display: flex;
      flex-direction: column;
      gap: 2px;
    ...
    /* Appel d'outil compact en ligne */
    .tool-call-inline {
      display: flex;
      align-items: center;
      gap: 6px;
    ...
    /* Carte legacy d'appel d'outil (restauration d'historique) */
    .tool-call {
      padding: 8px 12px;
      border-radius: var(--message-radius);
      background: var(--vscode-editorWidget-background);
    ...
    /* Wrapper du picker : position relative pour ancrer le menu deroulant */
    .picker-wrap {
      position: relative;
    }
    ...
    /* Menu deroulant du picker : sibling du bouton, positionne depuis le wrapper */
    .picker-dropdown {
      display: none;
      position: absolute;
      bottom: 100%;
    ...
    /* Espaceur de la barre d'outils */
    .toolbar-spacer { flex: 1; }

    /* Wrapper de l'editeur */
    .input-editor-wrap {
    ...
    /* Bouton bascule Envoyer/Stop en forme de pilule */
    .send-stop-btn {
      display: inline-flex;
      align-items: center;
      justify-content: center;
    ...
    /* Popup d'autocompletion des commandes slash */
    .slash-popup {
      display: none;
      position: absolute;
      bottom: 100%;
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    // Etat courant des modes/modeles
    let availableModes = [];
    let currentModeId = null;
    let availableModels = [];
    let currentModelId = null;

    // Etat de la section de reflexion
    ...
    // Etat des commandes slash disponibles
    ...
    function restoreState() {
      ...
      // Demande le rendu Markdown pour tous les messages assistant restaures
      if (assistantItems.length > 0) {
        vscode.postMessage({ type: 'renderMarkdown', items: assistantItems });
      }
    }

    // Commence avec la saisie desactivee
    if (inputArea) inputArea.classList.add('disabled');
    let currentAssistantEl = null;
    let currentAssistantText = '';
    let currentTurnEl = null;       // Conteneur .turn de la reponse en cours
    let currentToolsListEl = null;  // Liste .turn-tools-list du tour courant
    let currentToolsCountEl = null; // Compteur .turn-tools-summary du tour courant
    ...
    // --- Auto-redimensionnement du textarea (dans les contraintes de la zone de saisie) ---
    promptInput.addEventListener('input', () => {
      // Autocompletion des commandes slash
      ...
      if (text.startsWith('/') && availableCommands.length > 0) {
        ...
        if (firstSpace < 0) {
          // Nom de commande en cours de saisie : afficher une popup filtree
    ...
    // Envoi sur Entree (Shift+Entree pour un retour a la ligne)
    promptInput.addEventListener('keydown', (e) => {
      // Navigation clavier dans la popup slash
    ...
    // Branche les listeners des boutons
    ...
    function selectSlashCommand(cmd) {
      ...
      if (cmd.input) {
        // La commande attend une entree : inserer "/name " et un placeholder explicite
        promptInput.value = '/' + cmd.name + ' ';
        promptInput.placeholder = cmd.input.hint || 'Type input...';
        promptInput.focus();
      } else {
        // Aucune entree requise : envoi immediat
        promptInput.value = '/' + cmd.name;
        handleSend();
      }
    }

    // --- Pickers Mode / Modele (suite) ---
    ...
    // Ouvre/ferme les menus deroulants des pickers
    ...
    // Verifie que le tour courant possede un conteneur d'outils
    function ensureTurnTools() {
      if (!currentTurnEl) {
        // Cree un conteneur de tour si absent (ex.: appel d'outil avant le premier texte)
    ...
    // Constructeur DOM de fallback pour la restauration d'historique (carte autonome)
    ...
    function updateToolCall(toolCallId, status, title) {
      ...
      // Style inline (base sur les tours)
      ...
      // Fallback du style legacy en carte
    ...
    function finalizeThought() {
      ...
      if (thoughtEndTime) return; // Deja finalise, rien a faire
    ...
    function showSessionConnected(session) {
      ...
      // Met a jour les pickers a partir des donnees de session
      if (session.modes) updateModePicker(session.modes);
      if (session.models) updateModelPicker(session.models);
      // Restaure la liste des commandes disponibles
    ...
    function showNoSession() {
      ...
      if (inputArea) inputArea.classList.add('disabled');
      // Masque les pickers en cas de deconnexion
      modePickerWrap.classList.add('hidden');
      modelPickerWrap.classList.add('hidden');
    }

    // Gere les messages recus depuis l'extension host
    window.addEventListener('message', (event) => {
      ...
      switch (msg.type) {
        ...
        case 'promptEnd':
          // Finalise le bloc de reflexion s'il existe
          ...
          if (currentAssistantText) {
            ...
            saveState();
            // Demande le rendu Markdown a l'extension host
          ...
          currentAssistantEl = null;
          currentAssistantText = '';
          // Replie automatiquement les appels d'outils une fois le tour termine
        ...
        case 'markdownRendered': {
          // L'extension a renvoye le HTML rendu pour les messages
          const rendered = msg.items || [];
          for (const item of rendered) {
            // Retrouve l'element DOM correspondant a cet item d'historique
            // Pour le message stream termine a l'instant, met a jour le dernier element assistant
            const historyItem = chatHistory[item.index];
            if (!historyItem || historyItem.role !== 'assistant') continue;

            // Recherche l'element en parcourant tous les .message.assistant
            const allAssistant = messagesEl.querySelectorAll('.message.assistant');
            // item.index suit la position dans chatHistory ; ne compter que les messages assistant jusqu'a cet index
            let assistantIdx = 0;
            for (let i = 0; i < chatHistory.length; i++) {
      ...
      switch (type) {
        case 'agent_message_chunk': {
          ...
          if (content && content.type === 'text' && content.text) {
            ...
            // Ne cree pas d'element visible tant qu'il n'y a pas de contenu utile
            if (!currentAssistantEl && !currentAssistantText.trim()) {
              break;
            }
            // Replie automatiquement la reflexion des que le texte assistant commence
            if (currentThoughtEl && currentThoughtEl.open) {
              finalizeThought();
              currentThoughtEl.open = false;
            }
            if (!currentAssistantEl) {
              // Cree un conteneur de tour et y place le texte assistant
              if (!currentTurnEl) {
                currentTurnEl = document.createElement('div');
                currentTurnEl.className = 'turn';
                messagesEl.appendChild(currentTurnEl);
        ...
        case 'agent_thought_chunk': {
          ...
          if (content && content.type === 'text') {
            if (!currentThoughtEl) {
              // Cree le bloc de reflexion dans le tour courant
              if (!currentTurnEl) {
                currentTurnEl = document.createElement('div');
                currentTurnEl.className = 'turn';
                messagesEl.appendChild(currentTurnEl);
        ...
        case 'current_mode_update': {
          // Le serveur a pousse un changement de mode
    ...
    // Restaure l'etat precedent avant d'indiquer que le webview est pret
    restoreState();
