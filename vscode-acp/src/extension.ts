...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // Notifie le webview de chat quand la session active change
  sessionManager.on('active-session-changed', () => {
    chatWebviewProvider.notifyActiveSessionChanged();
  });

  // Vide le chat lorsqu'une nouvelle conversation demarre
  ...
  // Connecter un agent (action principale : icone inline dans l'arbre ou selection dans une liste)
  const connectAgentCmd = vscode.commands.registerCommand('acp.connectAgent', async (agentNameOrItem?: string | any) => {
    // Accepte un item d'arbre ou un nom d'agent brut (string)
    ...
    // Si on change d'agent alors que le chat contient des messages, demander confirmation
  ...
  // Nouvelle conversation (deconnexion + reset chat + reconnexion du meme agent)
  const newConversationCmd = vscode.commands.registerCommand('acp.newConversation', async () => {
    const activeSession = sessionManager.getActiveSession();
    if (!activeSession) {
      // Aucun agent actif : fallback vers la commande de connexion
      await vscode.commands.executeCommand('acp.connectAgent');
      return;
    }

    // Demande confirmation s'il existe deja du contenu dans le chat
  ...
  // Envoyer un prompt (depuis un raccourci : on focus simplement le chat)
  ...
  // Afficher le trafic protocolaire
  ...
  const removeAgentCmd = vscode.commands.registerCommand('acp.removeAgent', async (item?: any) => {
    ...
    // Deconnecte l'agent s'il est actuellement connecte
  ...
  // --- Enregistrer les ressources a liberer ---
  context.subscriptions.push(
    treeView,
